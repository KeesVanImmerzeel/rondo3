...
server <- function(input, output, session) {
  ...
  ############## End Functions from package rondo #######################################  
  
  # Calculate error = difference between observation (x, head) and model (m)
  .err <- function(x, Head, y) {
    return(Head - rd_phi(x, y))
  }
  err <- Vectorize(.err, vectorize.args = c("x", "Head"))
  
  # Calculate sum of absolute errors
  err_sum <- function(observations, y) {
    err(observations$x, observations$value, y) ^ 2 %>% sum()
  }
